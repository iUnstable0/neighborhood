...
export default async function handler(req, res) {
  ...
  if (!icon || !appId) {
    return res.status(400).json({ message: "Icon data and app ID are required" });
  }
  ...
  try {
    ...
    if (icon === 'placeholder') {
      ...
      console.log("Icon format:", icon ? icon.substring(0, 30) + "..." : "No icon");
    }
    ...
    // For data URLs, inform user that Airtable doesn't support direct uploads
      ...
      console.log("Data URL detected - Airtable doesn't support direct uploads");
      const placeholderUrl = "https://via.placeholder.com/100";
      ...
      try {
        ...
        return res.status(200).json({
          message: "Airtable doesn't support direct base64 uploads. Used placeholder instead.",
          iconUrl: placeholderUrl,
          warning: "To use your own icon, please provide a URL to a hosted image"
        });
      } catch (error) {
    ...
    if (error.stack) {
      ...
      console.error("Stack trace (first 3 lines):", error.stack.split('\n').slice(0, 3).join('\n'));
    }

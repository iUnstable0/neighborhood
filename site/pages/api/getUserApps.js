...
export default async function handler(req, res) {
  ...
  try {
    ...
    allApps.slice(0, 3).forEach((app, index) => {
      ...
      console.log(`Sample App ${index + 1}: ${app.id}, Name: ${app.fields.Name || 'Unnamed'}, ${iconInfo}, Neighbors:`, app.fields.Neighbors);
    });
    ...
    const userApps = allApps.filter(app => {
      ...
      if (hasUser) {
        console.log(`Found app ${app.fields.Name} with user in neighbors:`, neighbors);
      }
      
      return hasUser;
    });

    console.log(`Found ${userApps.length} apps for user after client-side filtering`);
    
    // If we didn't find any matching apps, also log apps with no neighbors
    if (userApps.length === 0) {
      const appsWithoutNeighbors = allApps.filter(app => !app.fields.Neighbors || app.fields.Neighbors.length === 0);
      console.log(`There are ${appsWithoutNeighbors.length} apps with no neighbors defined`);
      
      // Log these apps as they might be the ones we're looking for
      appsWithoutNeighbors.forEach((app, index) => {
        console.log(`App without neighbors ${index + 1}: ${app.id}, Name: ${app.fields.Name || 'Unnamed'}`);
      });
    }
    ...
    return res.status(500).json({ message: "Error fetching apps", error: error.message });
  }
} 
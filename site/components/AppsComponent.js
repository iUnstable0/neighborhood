...
const AppsComponent = ({
  ...
  const [appJoinableStates, setAppJoinableStates] = useState({});
  const toggleJoinable = async (appId) => {
    try {
      const response = await fetch("/api/toggleJoinable", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Failed to toggle joinable status");
      }

      const data = await response.json();
      setAppJoinableStates((prevStates) => ({
        ...prevStates,
        [appId]: data.is_joinable,
      }));
    } catch (error) {
      console.error("Error toggling joinable status:", error);
      alert("Failed to toggle joinable status");
    }
  };
  ...
  const handleIconUpload = async (e) => {
    ...
    if (file) {
      try {
        ...
        formData.append(
          "token",
          localStorage.getItem("neighborhoodToken") || getToken(),
        );

        // Upload to S3 via neighborhood-express
        const response = await fetch(
          "https://express.neighborhood.hackclub.com/upload-icon",
          {
            method: "POST",
            body: formData,
          },
  ...
  const handleImageUpload = async (e) => {
    ...
    if (files.length > 0) {
      try {
        ...
        // Upload to S3 via neighborhood-express
        const response = await fetch(
          "https://express.neighborhood.hackclub.com/upload-images",
          {
            method: "POST",
            body: formData,
          },
  ...
  const editApp = (app) => {
    ...
    console.log(
      "App's hackatime project GitHub links:",
      app.hackatimeProjectGithubLinks,
    );
    console.log(
      "Available hackatime projects:",
      ...
      if (isEditing && currentAppId) {
        ...
        if (!response.ok) {
          ...
          if (data.type === "project_already_attributed") {
            alert(
              `Cannot update app: The project "${data.projectName}" is already attributed to another app.`,
            );
            // Remove the project from selection
            setFormData((prev) => ({
              ...prev,
              hackatimeProjects: prev.hackatimeProjects.filter(
                (name) => name !== data.projectName,
              ),
              hackatimeProjectGithubLinks: {
                ...prev.hackatimeProjectGithubLinks,
          ...
          prevApps.map((app) =>
            app.id === currentAppId
              ? {
                  ...data.app,
                  hackatimeProjectGithubLinks:
                    data.app.hackatimeProjectGithubLinks || {},
                }
              : app,
          ),
        );
        ...
        if (!response.ok) {
          ...
          if (data.type === "project_already_attributed") {
            alert(
              `Cannot create app: The project "${data.projectName}" is already attributed to another app.`,
            );
            // Remove the project from selection
            setFormData((prev) => ({
              ...prev,
  ...
  const fetchAppDetails = async (appId) => {
    try {
      ...
      const response = await fetch(
        `/api/getAppDetails?token=${token}&appId=${appId}`,
      );
      if (!response.ok) {
  ...
  const handleAppClick = async (app) => {
    try {
      ...
      console.log(
        "GitHub links in app details:",
        appDetails?.hackatimeProjectGithubLinks,
      );
  ...
  useEffect(() => {
    const fetchApps = async () => {
      try {
        ...
        // Fetch joinable states for apps
        const fetchJoinableStates = async () => {
          const states = {};
          for (const app of apps) {
            const response = await fetch(
              `/api/checkJoinableStatus?appId=${app.id}`,
            );
            const data = await response.json();
            states[app.id] = data.is_joinable;
          }
          setAppJoinableStates(states);
        };
        ...
        if (sortedApps.length === 0) {
          ...
          console.log(
            "Apps found:",
            sortedApps.map((app) => app.name).join(", "),
          );
        }
  ...
  useEffect(() => {
    const fetchHackatimeProjects = async () => {
      try {
        ...
        const response = await fetch(
          `/api/getHackatimeProjects?slackId=${userData.slackId}&userId=${userData.id}`,
        );
        if (!response.ok) {
        ...
        const data = await response.json();
        console.log(
          "[DEBUG] Received Hackatime projects:",
          data.projects?.map((p) => ({
            name: p.name,
            isAttributed: p.isAttributed,
            attributedToAppId: p.attributedToAppId,
          ...
          {loading ? (
                    ...
                    >
                      {loadingHackatime ? (
                        ...
                        >
                          <div
                            style={{
                              ...
                              borderRadius: "50%",
                              animation:
                                "hackatimeLoadingSpin 1s linear infinite",
                            }}
                          />
                        ...
                        >
                          ...
                          >
                            Please connect your Slack account first to see your
                            Hackatime projects
                          </p>
                        </div>
                        ...
                        <>
                          ...
                          >
                            <input
                              ...
                              onChange={(e) =>
                                setHackatimeSearch(e.target.value)
                              }
                          ...
                          {hackatimeProjects
                            .filter((project) =>
                              project.name
                                .toLowerCase()
                                .includes(hackatimeSearch.toLowerCase()),
                            )
                            .slice(0, showAllProjects ? undefined : 5)
                            .map((project) => {
                              const isSelected =
                                formData.hackatimeProjects.includes(
                                  project.name,
                                );
                              const isAttributedToOtherApp =
                                project.isAttributed &&
                              ...
                              return (
                                <div
                                  ...
                                  style={{
                                    ...
                                    cursor: isAttributedToOtherApp
                                      ? "not-allowed"
                                      : "pointer",
                                    opacity: isAttributedToOtherApp ? 0.7 : 1,
                                    border: isSelected
                                      ? "1px solid #8b6b4a"
                                      : "1px solid #e0e0e0",
                                    transition: "all 0.2s",
                                ...
                                >
                                  <div
                                    onClick={() =>
                                      handleHackatimeProjectSelect(project)
                                    }
                                    style={{
                                      display: "flex",
                                      alignItems: "center",
                                    ...
                                    >
                                      ...
                                      >
                                        ...
                                        <input
                                          ...
                                          onChange={(e) => {
                                            console.log(
                                              "Updating GitHub link for project:",
                                              project.name,
                                            );
                                            console.log(
                                              "New value:",
                                            ...
                                            handleHackatimeProjectGithubLink(
                                              project.name,
                                              e.target.value,
                                            );
                                          }}
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            console.log(
                                              "Current GitHub links state:",
                                              formData.hackatimeProjectGithubLinks,
                                            );
                                            console.log(
                                              "Current value for",
                                              project.name,
                        ...
                        >
                          <div
                            style={{
                              ...
                              height: "3px",
                              background:
                                "linear-gradient(90deg, #8b6b4a 0%, #d4b595 100%)",
                              opacity: 0.8,
                            }}
                          />
                          <div
            ...
            >
              ...
              >
                ...
                >
                  Get started by creating a new app or joining an existing one
                </p>
                <div
                  style={{
                    display: "flex",
                    ...
                    onClick={() => {
                      ...
                      const fetchApps = async () => {
                        try {
                          ...
                          const response = await fetch(
                            `/api/getUserApps?token=${token}`,
                          );
                          const data = await response.json();

                          if (!response.ok) {
                            throw new Error(
                              data.message || "Failed to fetch apps",
                            );
                          }
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        if (!fixResponse.ok) {
                          throw new Error(
                            fixData.message || "Failed to fix app assignments",
                          );
                        }
                        ...
                        if (fixData.apps && fixData.apps.length > 0) {
                          alert(
                            `Successfully recovered ${fixData.apps.length} app(s). Refreshing list...`,
                          );
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        ...
                        right: "10px",
                        backgroundColor:
                          pendingLeaveAppId === app.id ? "#8b6b4a" : "#fff",
                        color:
                          pendingLeaveAppId === app.id ? "#fff" : "#8b6b4a",
                        border: "1px solid #8b6b4a",
                        borderRadius: "6px",
                        padding: "6px 10px",
                        fontSize: "12px",

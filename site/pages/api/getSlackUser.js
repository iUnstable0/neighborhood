  ...
  if (token) {
    try {
      ...
      if (!records || records.length === 0) {
        return res.status(404).json({ error: 'No neighbor found for this token' });
      }
      ...
      if (neighbor.slackNeighbor && Array.isArray(neighbor.slackNeighbor) && neighbor.slackNeighbor.length > 0) {
        try {
          const slackNeighborRec = await base('#neighborhoodSlackMembers').find(neighbor.slackNeighbor[0]);
      ...
      let slackData = {
        slackHandle: neighbor.slackHandle || slackNeighborFields['Slack Handle'] || null,
        fullName: neighbor.name || slackNeighborFields['Full Name'] || null,
        pfp: neighbor.pfp || (slackNeighborFields['Pfp'] && Array.isArray(slackNeighborFields['Pfp']) && slackNeighborFields['Pfp'][0]?.url) || null,
      ...
      return res.status(500).json({ error: 'Failed to fetch Slack user information by token' });
    }
  }
